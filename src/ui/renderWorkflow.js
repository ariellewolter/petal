    ...
    <div class="task-card workflow-task-card ${task.done ? 'done' : ''} ${blocked ? 'blocked' : ''}" 
      ...
      <div style="display:flex;align-items:flex-start;gap:8px;">
        <div class="check-box ${task.done ? 'checked' : ''}" 
             onclick="event.stopPropagation();if(window.Petal?.handlers?.toggleTask){window.Petal.handlers.toggleTask(${task.id})}"
             style="width:16px;height:16px;border:1.5px solid var(--border2);border-radius:50%;cursor:pointer;flex-shrink:0;margin-top:2px;background:white;transition:all .15s;${task.done ? 'background:linear-gradient(135deg,#d4a0a0,#c98b8b);border-color:#c98b8b;' : ''}"></div>
